...
const BinaryGame: React.FC<BinaryGameProps> = ({ addXP }) => {
  ...
  const renderEquation = () => {
    ...
    return (
        <div className="flex flex-wrap justify-center gap-2 font-mono text-sm text-neon-blue">
  ...
  return (
    <div className="flex flex-col items-center max-w-4xl mx-auto p-4 preserve-3d min-h-full pt-12">
      {/* Header Area */}
      <div className="mb-6 text-center w-full relative preserve-3d">
        <h2 className="text-5xl font-mono font-bold text-neon-blue mb-2 glitch-hover inline-block" style={{ textShadow: '0 0 20px rgba(0, 243, 255, 0.5)' }}>
            Binary Blitzer
        </h2>
        
        {/* Mode Switcher */}
        <div className="flex flex-wrap justify-center gap-4 mb-4 mt-4">
            <button onClick={() => setMode('builder')} className={`px-4 py-1 rounded border ${mode === 'builder' ? 'bg-neon-blue text-black border-neon-blue' : 'bg-black/50 text-gray-400 border-gray-700'}`}>Builder</button>
            <button onClick={() => setMode('decoder')} className={`px-4 py-1 rounded border ${mode === 'decoder' ? 'bg-neon-purple text-white border-neon-purple' : 'bg-black/50 text-gray-400 border-gray-700'}`}>Decoder</button>
            <button onClick={() => setMode('bitwise')} className={`px-4 py-1 rounded border ${mode === 'bitwise' ? 'bg-neon-green text-black border-neon-green' : 'bg-black/50 text-gray-400 border-gray-700'}`}>Bitwise</button>
        </div>
      </div>

      {/* Game Board (The Deck) */}
      <div className="bg-black/60 backdrop-blur-xl p-8 rounded-3xl border border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.5)] w-full max-w-3xl relative preserve-3d transform transition-transform duration-500">
        
        {/* Hard Mode Toggle - Elevated in 3D Space for clickability */}
        <div className="absolute top-6 right-6 z-50 transform translate-z-20">
            <button 
                onClick={(e) => {
                    e.stopPropagation();
                    setHardMode(!hardMode);
                }}
                className={`flex items-center gap-2 text-xs font-bold px-3 py-1 rounded border cursor-pointer transition-all hover:scale-105 ${hardMode ? 'bg-red-500/20 border-red-500 text-red-400 shadow-[0_0_10px_rgba(239,68,68,0.4)]' : 'bg-black/60 border-gray-600 text-gray-400 hover:text-white hover:border-white'}`}
            >
        ...
        {/* Target Display */}
        <div className="flex justify-between items-center mb-12 pt-4 preserve-3d">
            {mode === 'bitwise' ? (
                ...
                <div className="text-center flex-1">
                    ...
                    <div className="bg-black/50 border border-neon-blue/30 rounded-lg py-4 px-8 shadow-[inset_0_0_20px_rgba(0,0,0,0.8)]">
                        <span className="text-5xl font-bold text-white font-mono text-shadow-neon">
                            {mode === 'builder' ? target : <span className="text-neon-blue text-3xl tracking-[0.2em]">{targetBits.join('')}</span>}
                        </span>
                    </div>
                </div>
                
                <div className="px-4 text-gray-600"><ArrowRightLeft size={24} /></div>
                ...
                <div className="text-center flex-1 relative">
                    ...
                    {mode === 'builder' ? (
                        <div className={`bg-black/50 border rounded-lg py-4 px-8 shadow-[inset_0_0_20px_rgba(0,0,0,0.8)] transition-colors duration-300 ${currentBuilderValue === target ? 'border-neon-green text-neon-green' : 'border-gray-700 text-neon-purple'}`}>
                            <span className="text-5xl font-bold font-mono">{currentBuilderValue}</span>
                        </div>
                    ) : (
                        <div className="relative">
                            <input 
                                ...
                                className={`w-full bg-black/50 border-b-2 text-center text-5xl font-bold focus:outline-none py-2 transition-all ${isCorrect === false ? 'border-red-500 text-red-500 animate-shake' : formatError ? 'border-yellow-500 text-yellow-500' : 'border-neon-purple text-white focus:border-neon-green'}`}
                                placeholder="?"
                            />
                            {formatError && (
        ...
        {mode !== 'decoder' && (
             <div className={`flex justify-center items-center gap-2 sm:gap-3 mb-10 preserve-3d ${isCorrect === false ? 'animate-shake' : ''}`}>
                {(mode === 'builder' ? bits : bits).map((bit, index) => {
                    const value = Math.pow(2, 7 - index);
                    // 3D Cube Render
                    return (
                        <div key={index} className="relative w-8 h-12 sm:w-14 sm:h-20 cursor-pointer group cube-wrap" onClick={() => toggleBit(index)}>
                            <div className={`cube ${bit === 1 ? 'show-back' : 'show-front'}`}>
                                ...
                                <div className="cube-face cube-face-front bg-gray-800 border border-gray-600 rounded shadow-[inset_0_0_10px_rgba(0,0,0,1)] flex flex-col items-center justify-center group-hover:border-gray-400 transition-colors">
                                    <span className="text-gray-500 font-mono text-xl font-bold">0</span>
                                    {!hardMode && <span className="text-[9px] text-gray-600 mt-1">{value}</span>}
                                </div>
                                {/* Back Face (ON) */}
                                <div className="cube-face cube-face-back bg-neon-blue border-2 border-white/50 rounded shadow-[0_0_15px_#00f3ff] flex flex-col items-center justify-center">
                                    <span className="text-black font-mono text-xl font-bold">1</span>
                                    {!hardMode && <span className="text-[9px] text-black/70 mt-1 font-bold">{value}</span>}
                                </div>
                            </div>
                        </div>
                    );
        ...
        {/* Decoder Static Bits */}
        {mode === 'decoder' && (
            <div className="flex justify-center gap-2 mb-10 preserve-3d">
                {targetBits.map((bit, index) => (
                    <div key={index} className={`
                        w-10 h-14 flex flex-col items-center justify-center rounded border-2 font-bold text-xl shadow-lg
        ...
        <div className="flex flex-col gap-6 relative z-10">
           ...
           <div className="flex justify-center gap-4">
            <button
                ...
                className={`px-10 py-4 rounded-full font-bold text-lg uppercase tracking-widest transition-all shadow-lg transform active:scale-95
              ...
              {isCorrect && (
                <button onClick={generateTarget} className="p-4 rounded-full bg-gray-800 text-white hover:bg-gray-700 border border-gray-600 animate-pop">
            ...
            <div className="min-h-[100px] text-center">
              {message && (
                <div className={`inline-flex items-center gap-3 px-6 py-2 rounded-lg border font-bold text-lg animate-pop backdrop-blur-sm ${isCorrect ? 'bg-neon-green/20 border-neon-green text-neon-green' : 'bg-red-500/20 border-red-500 text-red-500'}`}>
                  {isCorrect ? <CheckCircle /> : <XCircle />} {message}
                </div>
              )}
