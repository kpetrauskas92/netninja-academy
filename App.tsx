...
import ExploitLabGame from './components/ExploitLabGame';
import PacketSnifferGame from './components/PacketSnifferGame';
...
import { Terminal, Shield, Cpu, BookOpen, User, Award, Lock, X, Check, HelpCircle, GraduationCap, PlayCircle, Globe, Calendar, Flame, ShoppingBag, Sliders, Lock as LockIcon, Zap, Search, Wifi, Menu, Network } from 'lucide-react';
...
const App = () => {
      ...
      <div key={currentMode} className="animate-fade-in-up h-full">
        {(() => {
          switch (currentMode) {
            ...
            case GameMode.EXPLOIT_LAB:
              return <ExploitLabGame addXP={addXP} />;
            case GameMode.PACKET_SNIFFER:
              return <PacketSnifferGame addXP={addXP} />;
  ...
  return (
    <div className="perspective-scene bg-neon-dark overflow-hidden">
      {/* The World Container */}
      <div className="world-3d flex h-full">
          ...
          {/* Mobile Nav Overlay */}
          {isMobileNavOpen && (
            <div className="fixed inset-0 bg-black/80 z-40 md:hidden backdrop-blur-sm" onClick={() => setIsMobileNavOpen(false)}></div>
          )}

          {/* Sidebar Navigation */}
          <nav className={`
            fixed inset-y-0 left-0 z-50 w-72 bg-[#0b0b14] border-r border-white/10 shadow-[5px_0_30px_rgba(0,0,0,0.5)] flex flex-col
            transform transition-transform duration-300 ease-out
            ${isMobileNavOpen ? 'translate-x-0' : '-translate-x-full'}
            md:relative md:translate-x-0
          `}>
            {/* Top Section: Logo + Menu */}
            <div className="flex-1 flex flex-col min-h-0 p-5 pb-0">
              <div 
                className="flex items-center gap-3 mb-8 cursor-pointer group shrink-0" 
                onClick={() => handleNavClick(GameMode.DASHBOARD)}
              >
                <div className="bg-neon-blue p-1.5 rounded shadow-[0_0_15px_rgba(0,243,255,0.4)] group-hover:shadow-[0_0_25px_rgba(0,243,255,0.8)] transition-all duration-300 group-hover:rotate-[360deg]">
                ...
                <button className="ml-auto md:hidden text-gray-500" onClick={(e) => { e.stopPropagation(); setIsMobileNavOpen(false); }}><X size={24}/></button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto scrollbar-hide space-y-1 pr-2">
                ...
                />
                 <NavButton 
                    ...
                    onClick={() => handleNavClick(GameMode.SHOP)} 
                    icon={<ShoppingBag />} 
                    label="Cyber Deck Store" 
                />
                
                <div className="mt-4 mb-2 text-[11px] font-bold text-gray-500 uppercase tracking-widest pl-3">Training Protocols</div>
                
                <NavButton 
                    active={currentMode === GameMode.TUTORIAL} 
                    onClick={() => handleNavClick(GameMode.TUTORIAL)} 
                    icon={<GraduationCap />} 
                    label="Tutorials" 
                />
                <NavButton 
                    active={false} 
                    onClick={() => { setIsStudyOpen(true); setIsMobileNavOpen(false); }} 
                    icon={<HelpCircle className="text-neon-green" />} 
                    label="Field Manual" 
                />

                <div className="mt-4 mb-2 text-[11px] font-bold text-gray-500 uppercase tracking-widest pl-3">Simulations</div>
                <NavButton 
                    active={currentMode === GameMode.BINARY} 
                    onClick={() => handleNavClick(GameMode.BINARY)} 
                    icon={<Cpu />} 
                    label="Binary Blitzer" 
                />
                <NavButton 
                    active={currentMode === GameMode.HEX} 
                    onClick={() => handleNavClick(GameMode.HEX)} 
                    icon={<Shield />} 
                    label="Hex Hero" 
                />
                <NavButton 
                    active={currentMode === GameMode.SUBNET} 
                    onClick={() => handleNavClick(GameMode.SUBNET)} 
                    icon={<Network />} 
                    label="Subnet Showdown" 
                />
                 <NavButton 
                    active={currentMode === GameMode.PACKET_TRACER} 
                    onClick={() => handleNavClick(GameMode.PACKET_TRACER)} 
                    icon={<Globe />} 
                    label="Packet Tracer" 
                />
                 <NavButton 
                    active={currentMode === GameMode.SANDBOX} 
                    onClick={() => handleNavClick(GameMode.SANDBOX)} 
                    icon={<Sliders className="text-neon-pink" />} 
                    label="Sandbox Mode" 
                />

                 <div className="mt-4 mb-2 text-[11px] font-bold text-gray-500 uppercase tracking-widest pl-3">Security Operations</div>
                 <NavButton 
                    active={currentMode === GameMode.FIREWALL} 
                    onClick={() => handleNavClick(GameMode.FIREWALL)} 
                    icon={<Zap className="text-red-500" />} 
                    label="Firewall Frenzy" 
                />
                <NavButton 
                    active={currentMode === GameMode.THREAT_HUNTER} 
                    onClick={() => handleNavClick(GameMode.THREAT_HUNTER)} 
                    icon={<Search className="text-neon-blue" />} 
                    label="Threat Hunter" 
                />
                <NavButton 
                    active={currentMode === GameMode.PACKET_SNIFFER} 
                    onClick={() => handleNavClick(GameMode.PACKET_SNIFFER)} 
                    icon={<Wifi className="text-neon-blue" />} 
                    label="Packet Sniffer" 
                />
                 <NavButton 
                    active={currentMode === GameMode.EXPLOIT_LAB} 
                    onClick={() => handleNavClick(GameMode.EXPLOIT_LAB)} 
                    icon={<Terminal className="text-neon-green" />} 
                    label="Exploit Lab" 
                />
              </div>
            </div>

            {/* Player Stats Mini - Fixed at bottom */}
            <div className="p-5 bg-[#0b0b14] border-t border-white/10 shrink-0 z-10 mt-2">
                <div className="bg-white/5 p-3 rounded-lg border border-white/5 hover:bg-white/10 transition-colors cursor-default">
                    <div className="flex items-center gap-3 mb-2">
                        <div className="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center border border-neon-purple/50 shadow-[0_0_10px_rgba(188,19,254,0.3)] text-xl">
                            {currentAvatarIcon}
                        </div>
                        <div className="overflow-hidden">
                            <div className="text-sm font-bold text-white truncate">Cadet Level {stats.level}</div>
                            <div className="text-xs text-gray-400">{stats.xp} XP</div>
                        </div>
                    </div>
          ...
          {/* Main Content Area */}
          <main className="flex-1 relative overflow-y-auto h-full perspective-scene w-full">
             {/* Top Bar Mobile Only */}
             <div className="md:hidden flex justify-between items-center p-4 border-b border-white/10 bg-black/80 backdrop-blur-md sticky top-0 z-30 shadow-lg">
                <div className="flex items-center gap-3">
                    <button onClick={() => setIsMobileNavOpen(true)} className="text-white hover:text-neon-blue transition-colors">
                        <Menu size={24} />
                    </button>
                    <span className="font-bold text-neon-blue tracking-tighter">NETNINJA</span>
                </div>
                <div className="flex items-center gap-4">
                   <button onClick={() => setIsStudyOpen(true)} className="text-neon-green">
                      <BookOpen size={20} />
                   </button>
                   <div className="text-xs text-white font-mono border border-white/20 px-2 py-1 rounded bg-black/40">Lvl {stats.level}</div>
                </div>
             </div>

            <div className="p-4 md:p-12 max-w-7xl mx-auto pb-24 min-h-full preserve-3d">
                {renderContent()}
            </div>
          </main>
      </div>
...
const NavButton = ({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) => (
    <button
        onClick={onClick}
        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md transition-all duration-200 group relative overflow-hidden shrink-0 text-sm ${
            active 
            ? 'bg-neon-blue/10 text-white border-l-2 border-neon-blue shadow-[0_0_20px_rgba(0,243,255,0.2)] translate-x-1' 
            : 'text-gray-400 hover:text-white hover:bg-white/5 hover:translate-x-1'
        }`}
    >
        <span className={`relative z-10 transition-transform group-hover:scale-110 ${active ? 'text-neon-blue' : ''}`}>
...
const Dashboard = ({ changeMode, stats, dailyCompleted }: { changeMode: (m: GameMode) => void, stats: PlayerStats, dailyCompleted: boolean }) => {
  ...
  return (
    <div className="space-y-8 md:space-y-12 preserve-3d">
        {/* Badge Modal */}
        {selectedBadge && (
        ...
        {/* Hero Section */}
        <div className="bg-gradient-to-r from-neon-card to-black p-6 md:p-8 rounded-2xl border-l-4 border-neon-blue shadow-2xl relative overflow-hidden animate-slide-in-right preserve-3d transform hover:translate-z-10 transition-transform duration-500">
            ...
            <div className="relative z-10">
                <h2 className="text-3xl md:text-4xl font-bold text-white mb-2 tracking-tight">Welcome back, Cadet.</h2>
                <p className="text-gray-400 max-w-xl text-sm md:text-lg">Your neural link is active. Select a training module to begin your journey.</p>
            </div>
            
            {/* Stats Bar */}
            <div className="mt-8 flex flex-wrap items-center gap-4 md:gap-8 relative z-10">
                <div className="flex items-center gap-3 bg-black/40 px-4 py-2 rounded-lg border border-white/5 flex-1 md:flex-none min-w-[120px]">
                    <Flame className="text-orange-500 fill-orange-500" size={20} />
                    <div>
                        <div className="text-xl md:text-2xl font-bold text-white leading-none">{stats.streak}</div>
                 ...
                 <div className="flex items-center gap-3 bg-black/40 px-4 py-2 rounded-lg border border-white/5 flex-1 md:flex-none min-w-[120px]">
                    ...
                    <div>
                        <div className="text-xl md:text-2xl font-bold text-white leading-none">{stats.badges.length}</div>
                        <div className="text-[10px] text-gray-400 uppercase font-bold">Badges</div>
                    </div>
                </div>
                <div className="flex items-center gap-3 bg-black/40 px-4 py-2 rounded-lg border border-white/5 cursor-pointer hover:bg-black/60 transition-colors flex-1 md:flex-none min-w-[120px]" onClick={() => changeMode(GameMode.SHOP)}>
                    <ShoppingBag className="text-neon-pink" size={20} />
                    <div>
                        <div className="text-xl md:text-2xl font-bold text-white leading-none">{stats.xp} XP</div>
        ...
        <div className="animate-fade-in-up delay-75 preserve-3d">
             <div className="flex items-center gap-2 mb-4 text-white font-bold text-lg md:text-xl"><Calendar className="text-yellow-500" /> Daily Protocol</div>
             <div 
                onClick={() => !dailyCompleted && changeMode(GameMode.DAILY_CHALLENGE)}
                className={`
                    relative p-6 rounded-xl border transition-all duration-300 group flex flex-col md:flex-row items-center justify-between shadow-lg preserve-3d gap-4 md:gap-0
             ...
             >
                 <div className="flex items-center gap-6 w-full md:w-auto">
                     <div className={`p-4 rounded-xl transition-colors duration-300 ${dailyCompleted ? 'bg-gray-800 text-gray-500' : 'bg-yellow-500/10 text-yellow-500 group-hover:bg-yellow-500 group-hover:text-black'}`}>
                         {dailyCompleted ? <Check size={32} /> : <Flame size={32} />}
                     </div>
                     <div>
                         <h3 className={`text-xl md:text-2xl font-bold transition-colors ${dailyCompleted ? 'text-gray-500' : 'text-white group-hover:text-yellow-500'}`}>
                 ...
                 <div className="text-right w-full md:w-auto">
                     {dailyCompleted ? (
                         <span className="text-xs font-bold text-gray-600 border border-gray-700 px-3 py-1 rounded block md:inline text-center">COMPLETE</span>
                     ) : (
                         <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-start w-full gap-2">
                             <span className="text-yellow-500 font-bold md:mb-1">+300 XP</span>
        ...
        <div className="animate-fade-in-up delay-100 preserve-3d">
             <div className="flex items-center gap-2 mb-4 text-white font-bold text-lg md:text-xl"><GraduationCap className="text-neon-pink" /> Training Grounds</div>
             <div 
                onClick={() => changeMode(GameMode.TUTORIAL)}
                className="bg-black/40 backdrop-blur-md p-6 rounded-xl border border-gray-700 hover:border-neon-pink cursor-pointer transition-all duration-300 group flex flex-col md:flex-row items-center justify-between shadow-lg hover:shadow-neon-pink/20 transform hover:translate-x-2 gap-4 md:gap-0"
             >
                 <div className="flex items-center gap-6 w-full md:w-auto">
                     ...
                     <div>
                         <h3 className="text-xl md:text-2xl font-bold text-white group-hover:text-neon-pink transition-colors">Interactive Tutorials</h3>
                         <p className="text-sm text-gray-400">Step-by-step guided missions. Start here.</p>
                     </div>
                 </div>
                 <div className="text-neon-pink opacity-100 md:opacity-0 group-hover:opacity-100 transition-all duration-300 transform md:translate-x-8 group-hover:translate-x-0">
                     <PlayCircle size={32} />
                 </div>
             </div>
        </div>
        ...
        {/* Game Modules */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 animate-fade-in-up delay-300 preserve-3d">
            <DashboardCard 
            ...
            />
             <DashboardCard 
                title="Exploit Lab" 
                desc="Vuln scanning & Payload delivery."
                color="border-neon-green"
                icon={<Terminal size={32} className="text-neon-green" />}
                onClick={() => changeMode(GameMode.EXPLOIT_LAB)}
            />
            <DashboardCard 
                title="Packet Sniffer" 
                desc="Analyze traffic for anomalies."
                color="border-neon-blue"
                icon={<Wifi size={32} className="text-neon-blue" />}
                onClick={() => changeMode(GameMode.PACKET_SNIFFER)}
            />
            <DashboardCard 
                title="Sandbox Mode" 
                desc="Freeform experimentation."
                color="border-neon-pink"
                icon={<Sliders size={32} className="text-neon-pink" />}
                onClick={() => changeMode(GameMode.SANDBOX)}
            />
        </div>
    </div>
  );
...
const DashboardCard = ({ title, desc, color, icon, onClick }: any) => (
    <div 
        onClick={onClick}
        className={`
            bg-black/60 backdrop-blur-sm p-6 md:p-8 rounded-2xl border-t-4 ${color} 
            ...
            preserve-3d flex items-center gap-4 md:block
        `}
    >
        <div className="bg-gray-800/50 w-12 h-12 md:w-16 md:h-16 flex items-center justify-center rounded-xl md:rounded-2xl group-hover:scale-110 group-hover:rotate-6 transition-transform duration-300 shadow-inner mb-0 md:mb-6 shrink-0">
            {icon}
        </div>
        <div>
            <h3 className="text-lg md:text-xl font-bold text-white mb-1 md:mb-2 group-hover:text-neon-blue transition-colors">{title}</h3>
            <p className="text-xs text-gray-400 leading-relaxed">{desc}</p>
        </div>
        <div className="absolute bottom-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity transform translate-y-4 group-hover:translate-y-0 hidden md:block">
